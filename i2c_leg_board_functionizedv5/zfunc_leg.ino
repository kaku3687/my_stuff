...
#define FSR_POLL_SPEED             20
#define FSR_NUM                    1              //One or two fsrs. Uses pin_analog_read_1, pin_analog_read_2
#define FSR_POLL_LIMIT             3              //number of times this fsr could be read
...
byte rot_pin;
byte lim_pin;
byte fsr_pin_1 = pin_analog_read_1;
byte fsr_pin_2 = pin_analog_read_0;
...
unsigned char ifsr_degree_increments = 3;
unsigned char ifsr_threshold_percentage = 5;
...
boolean rot_count_over_and_limit_enabled, rot_count_equal_or_under_0;
...
void z_rising(){  
    if( ((millis() - z_previous_time) > z_debounce_time) &  (digitalRead(rot_pin) == HIGH)){
        ...
        if(status == LEG_FUNC_STATUS_BUSY){
          if(((rot_count - rot_goal) == 1 )| ((rot_count - rot_goal) == -1) & !incremental_move & !stabilization ){    //Slows down motor before hitting last limit
            set_speed(0, slowMotorSpeed);
          }
          ...
          else if( ((rot_count >= rot_limit) & rot_limit_enabled) & !incremental_move & !stabilization ){
...
void fsr_check(){
  ...
  if(incremental_move | stabilization){
    ...
    //rot_count_over_and_limit_enabled = (rot_count >= rot_limit) & rot_limit_enabled;
    rot_count_over_and_limit_enabled = (rot_count >= (rot_limit - 2)) & rot_limit_enabled;
    ...
    rot_count_equal_or_under_0 = (rot_count <= (setHt - 2));
