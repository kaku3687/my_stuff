...
void z_rising(){  
    if( ((millis() - z_previous_time) > z_debounce_time) &  (digitalRead(rot_pin) == HIGH)){
        ...
        if(!stalled){                                                              //If motor is not stalled, then this is a complete count
        ...
        else{                                                                      //Motor was stalled, therefore the motor was driven backwards, negate this count
          walk_timeout_complete();                                                //Stop motor
                                                                                  //Stall switch is cleared
        }
